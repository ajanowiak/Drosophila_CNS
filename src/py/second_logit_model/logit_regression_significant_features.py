...
Pipeline context: Snakemake fans out over shap_model, tissue, and epv (the
same epv used to produce the regression_coefs.py output this stage reads).
...
Outputs:
  ...
  - results/second_logit_model/<shap_model>/epv_<epv>/<tissue>_cv_results.csv
  - results/figures/second_logit_model/<shap_model>/epv_<epv>/<tissue>_coefficients.pdf
  - results/figures/second_logit_model/<shap_model>/epv_<epv>/<tissue>_volcano.pdf
"""
...
def run_significant_features(
    ...
    """Fit and save the second logit model for one (tissue, shap_model, epv)."""
    logger.info(f"Significant features: {tissue} | shap_model={shap_model} | epv={epv}")

    significant_features = extract_significant_features(tissue, shap_model, epv, p_value_threshold)
    ...
    data_dir = f"results/second_logit_model/{shap_model}/epv_{epv}"
    fig_dir = f"results/figures/second_logit_model/{shap_model}/epv_{epv}"
...
def main() -> None:
    ...
    run_significant_features(
        args.tissue, args.shap_model, args.epv, args.n_splits,
